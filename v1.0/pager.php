	...
	function generate($links = null, $ignore = null, $format = true)
	{
		...
		if(($start - 1) >= 1) {
			$this->pages['first'] = $this->tools['first']
				? '<a class="first" href="'. $uri .'/'. $this->segment .'/1">'. $this->tools['first'] .'</a>'
				: '';
			$this->pages['prev'] = '<a class="prev" href="'. $uri .'/'. $this->segment .'/'. ($start - 1) .'">'. $this->tools['prev'] .'</a>';
		}
		...
		if($start != $this->total_pages) {
			$this->pages['next'] = '<a class="next" href="'. $uri .'/'. $this->segment .'/'. ($start + 1) .'">'. $this->tools['next'] .'</a>';
			$this->pages['last'] = $this->tools['last']
				? '<a class="last" href="'. $uri .'/'. $this->segment .'/'. $this->total_pages .'">'. $this->tools['last'] .'</a>'
				: '';
		}
...
/* Location: ./application/libraries/pager.php */